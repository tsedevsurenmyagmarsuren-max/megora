...
export default async function InternationalLessonPage({
  params,
}: {
  ...
  const lesson = internationalLessons.find((item) => item.slug === slug);
  ...
  return (
    <main className="min-h-screen bg-gray-50">
      <div className="bg-cyan-600 text-white py-10">
        <div className="max-w-5xl mx-auto px-6">
          <h1 className="text-4xl font-bold">{lesson.title}</h1>
          <p className="mt-2 text-cyan-100">{lesson.description}</p>
        </div>
      </div>
